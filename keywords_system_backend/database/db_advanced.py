    ...
    if (type(result1) == int and result1 > 0) or result1 == '':
        ...
        if idPrefix == '':
            ...
            for ele in result2:
                ...
                initID += 1
        # print('xxxx',result2)
    return ({'count': result1, 'content': result2,'dbName': dbName})
...
async def updateCrawlerInfo(dbName, collectionName,queryDict={}, setDict={}, pageSize=10, currentPage=1):
    """
    更新 CrawlerInfo 信息
    """
    result1 = await update_one(dbName, collectionName, queryDict=queryDict, setDict=setDict)
...
async def fetchCrawlerInfo(dbName, collectionName):
    """
    获取服务端保存的 爬虫任务的 最新一个
    """
    result1 = await fetchTable(dbName, collectionName, xshown={'_id':0,'taskID':1,'status': 1,'targets':1},xsort=[('_id',-1)],  pagesize=1, returnTotalCount=False)
    return result1


async def addCrawlerInfo(dbName, collectionName, ItemInfos={}):
    """
     记录 每一次 爬虫的 部分信息
    """
     # 直接使用 insert_many
    #print('ItemInfos',dbName, collectionName,ItemInfos)
    try:
        result1 =  await insert_one(dbName, collectionName, data=ItemInfos)
    except Exception as e:
        print(e)
        raise
    else:
...
async def updateUrlItems(dbName, collectionName, queryDict={}, setDict={}, pageSize=10, currentPage=1):
    ...
    #print('++++++++++++++', dbName, collectionName,     setDict, queryDict, pageSize, currentPage)
    result1 = await update_one(dbName, collectionName, queryDict=queryDict, setDict=setDict)
    # print('result1',result1)
    if result1 == 1:
        #print('插入成功')
...
async def fetchDictItems(dbName, collectionName, xfilter={}, xshown={}, currentpage=1, xsort=[],pagesize=10, returnTotalCount=True):
    # 获取 特定项目 Url表中符合条件的数据
...
async def updateDigInfo(dbName, collectionName,queryDict={}, setDict={}, pageSize=10, currentPage=1):
    """
    更新 DigInfo 信息
    """
    result1 = await update_one(dbName, collectionName, queryDict=queryDict, setDict=setDict)
...
async def fetchDigInfo(dbName, collectionName):
    """
    获取服务端保存的 挖词任务的 最新一个
    """
    result1 = await fetchTable(dbName, collectionName, xshown={'_id':0,'taskID':1,'status': 1},xsort=[('_id',-1)],  pagesize=1, returnTotalCount=False)
    return result1



async def addDigInfo(dbName, collectionName, ItemInfos={}):
    """
     记录 每一次 挖词的 部分信息
    """
     # 直接使用 insert_one
...
async def updateExpandInfo(dbName, collectionName,queryDict={}, setDict={}, pageSize=10, currentPage=1):
    """
    更新 ExpandInfo 信息
    """
    result1 = await update_one(dbName, collectionName, queryDict=queryDict, setDict=setDict)
...
async def fetchExpandInfo(dbName, collectionName):
    """
    获取服务端保存的 拓词 任务的 最新一个
    """
    result1 = await fetchTable(dbName, collectionName, xshown={'_id':0,'taskID':1,'status': 1},xsort=[('_id',-1)],  pagesize=1, returnTotalCount=False)
    return result1
    
async def addExpandInfo(dbName, collectionName, ItemInfos={}):
    """
     记录 每一次 拓词的 部分信息
    """
     # 直接使用 insert_one
...
async def makeAggregations(dbName, collectionName, currentPage=1, pageSize = 10, xaggregate= [],types='topicWord',returnTotalCount=True):
    ...
    if (type(result1) == int and result1 > 0) or result1 == '':
        ...
        result2 = await aggregate(dbName, collectionName, aggregation=xaggregate)
...
async def fetchExtendedWordsTopic(dbName, collectionName,  currentPage=1, pageSize = 10,xaggregate =[],returnTotalCount=True):
    ...
    # print(result1,type(result1))
    result1['content'] = list( json.loads(json_util.dumps(result1['content'])))
    return (result1)

async def jisuanInheritData(dbName, collectionName,  currentPage=1, pageSize = 10,xaggregate =[],returnTotalCount=True):
    # 获取 特定项目 extendedWords 表中符合条件的  母词  数据 ,只通过group得到所有的母词 列表
    result1 = await makeAggregations (dbName, collectionName, currentPage=0, pageSize = 0, types="mword", xaggregate=xaggregate, returnTotalCount=True)
    # print(result1,type(result1))
    return result1

async def fetchExtendedWordsInherit(dbName, collectionName,  currentPage=1,pageSize = 10,xfilter={},xsort={},xshown ={},currentpage=1, pagesize=10,returnTotalCount=True):

    # 获取 特定项目 INHERITagg 表中符合条件的数据
    # print(dbName,collectionName,xfilter,xshown)
    result1 = await fetchTable(dbName, collectionName, xfilter=xfilter, xshown=xshown, xsort=xsort, currentpage=currentpage, pagesize=pagesize, returnTotalCount=True)
    # print(result1,'ccccccc')
    return (result1)
...
async def fetchExtendedWordsInheritAgg(dbName, collectionName,  currentPage=1, pageSize = 10,xaggregate =[],returnTotalCount=True):
    # 获取 特定项目 extendedWords 表中符合条件的  母词 的 聚合 数据 
    result1 = await makeAggregations (dbName, collectionName, currentPage=currentPage, pageSize = pageSize, types="mword", xaggregate=xaggregate, returnTotalCount=True)
    # print(result1,type(result1))
    result1['content'] = list(result1['content'])
    return result1

async def fetchExtendedWordsTopicAgg(dbName, collectionName,  currentPage=1, pageSize = 10,xaggregate =[],returnTotalCount=True):
    # 获取 特定项目 topicAgg 表中符合条件的  主题词 的 聚合 数据 
    result1 = await makeAggregations (dbName, collectionName, currentPage=currentPage, pageSize = pageSize, types="mword", xaggregate=xaggregate, returnTotalCount=True)
    # print(result1,type(result1))
    result1['content'] = list(result1['content'])
    return result1

async def deleteFromTopicAggCollection(dbName, collectionName, deleteList=[], currentPage=1, pageSize=10):
    # 解散 主题词
    if deleteList == []:
        # 什么也不删除
        return ('error')
        ...
        try:
            for ele in deleteList:
                ...
                result1 = await delete_one(dbName, collectionName, ele)
            # 成功，刷新列表
            result2 = await fetchTable(dbName, collectionName,currentpage=currentPage, pagesize=pageSize, returnTotalCount=True)
            return (result2)
        except:
            return ('error')
...
async def add2InheritAggCollection(dbName, collectionName, xfilter={},xset={}):
    # 将 母词聚合数据写入或更新到 表 inheritAgg
    try:
        result1 = await update_one(dbName, collectionName, queryDict=xfilter, setDict=xset)
    except:
        raise
    else:
        return result1
...
async def add2TopicAggCollection(dbName, collectionName, xfilter={},xset={}):
    # 将 主题词聚合数据写入或更新到 表 topicAgg
    try:
        result1 = await update_one(dbName, collectionName, queryDict=xfilter, setDict=xset)
    except:
        raise
