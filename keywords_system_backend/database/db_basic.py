...
try:
    ...
    client = pymongo.MongoClient('mongodb://root:root@localhost:27017/',unicode_decode_error_handler='ignore')
    #client = pymongo.MongoClient('mongodb://root:root@114.67.113.229:8004/',unicode_decode_error_handler='ignore')
    #client = pymongo.MongoClient('mongodb://root:sccgoodToGo@114.67.109.244:27017/',unicode_decode_error_handler='ignore')
    print('mongodb连接成功')
...
def dbinit():
    ...
    connections.create_connection(alias=esconnection, hosts=['http://localhost:9200'], timeout=60,max_retries=10, retry_on_timeout=True)
    #connections.create_connection(alias=esconnection, hosts=['http://114.67.113.229:8001'], timeout=60, max_retries=10, retry_on_timeout=True)
...
async def insert_one(dbName, collectionName, data={}):
    ...
    if collectionName == 'Categories':
        ...
        collectionHandler.create_index(
            [('url', 1)], unique=True, background=True)
    elif collectionName in ['StopDict', 'InvalidDict', 'UserDict','inheritAgg']:
        collectionHandler.create_index(
            [('word', 1)], unique=True, background=True)
...
async def update_one(dbName, collectionName, queryDict={}, setDict={}):
    ...
    # 所有异常交给上面
    try:
        result1 = collectionHandler.update_one(queryDict, setDict, upsert=True)  # 有了upsert 之后，就算没有找到目标，也会插入，所以会显示返回0
        return (result1.modified_count) # 有了upsert 之后，就算没有找到目标，也会插入，所以会显示返回0
    except:
        raise
