...
<div>
    <div class="extendedWords-button-group">
        ...
        <div class="p2">
            <div class="p22">
                ...
                <i-page class="page" :total="itemCount" placement="bottom" :current="currentPage" :page-size="pageSize" :page-size-opts=[10,20,30,40,50,100] size="small" show-elevator show-total show-sizer @on-change="pageChange" @on-page-size-change="pageSizeChange" />
            </div>
    ...
    <i-page class="page" :total="itemCount" placement="top" :current="currentPage" :page-size="pageSize" :page-size-opts=[10,20,30,40,50,100] size="small" show-elevator show-total show-sizer @on-change="pageChange" @on-page-size-change="pageSizeChange" />
    ...
    methods: {
        ...
        singleSearch: function (item) {
            ...
            let Params = {
                ...
                'pageSize': self.pageSize,
                'extendedWordItemId': item[0] // item 就是uid 此处
            }
        ...
        batchSearch: function (wordItem) {
            ...
            // 下面进行 batchSearch
            self.currentPage = 1
            self.changeSearchDisplay(wordItem)
        ...
        deleteItems: function () {
            ...
            // console.log(this.selectedItemList)
            // 当没有选项被选中时，不进行该操作
            let uids = [] // 经过几次修改，uids 不仅包含uid，也包含 topicWord 和 mword，因为删除操作，需要重新计算先关 统计值
            //let toDismissList = [] // 如果是删除 主题词本身的词，相当于 删除 + dismiss
            ...
            for (let element in self.selectedItemList) {
                ...
                let topicWord = ''
                if (self.selectedItemList[element]['topicWord'] === self.selectedItemList[element]['word']){
                        //self.dismiss(self.selectedItemList[element]['_id']['$oid'],self.selectedItemList[element]['word'])
                        //toDismissList.push([self.selectedItemList[element]['_id']['$oid'],self.selectedItemList[element]['word']])
                        self.dismiss(self.selectedItemList[element]['_id']['$oid'],self.selectedItemList[element]['word'])
                        break
                }else{
                    topicWord = self.selectedItemList[element]['topicWord']
