    ...
    methods: {
        generateData: function (pageData2Export) {
            ...
            for (let ele in pageData2Export) {
                ...
                temp.push(lineData.source)
                temp.push(lineData.mword)
                temp.push(lineData.Length)
                temp.push(lineData.baiduIndex)
                temp.push(lineData.searchCount)
                temp.push(lineData.bidPrice)
                temp.push(lineData.baiduComment)
                temp.push(lineData.usageTag.join(','))
                toPrint.push(temp)
            }
            return toPrint
        },
        modalOk() {
            ...
            let toPrint = []
            // 设置 header
            toPrint.push(['id', '拓展词', '状态', '分类', '时间', '出处', '母词', '长度', '百度指数', '搜索次数', '竞价', '百度备注', '用途标签'])
            toPrint.push(['', '', '', '', '', '', '', '', '', '', '', '', ''])
            ...
            if (self.selected3 !== '当前选中' && self.selected3 !== '当前页') {
                ...
                self.axios({
                    ...
                    .then(res => {
                        ...
                        let worksheet = self.$XLSX.utils.aoa_to_sheet(toPrint)
                        let new_workbook = self.$XLSX.utils.book_new()
                        self.$XLSX.utils.book_append_sheet(new_workbook, worksheet, "扩展词典数据")
                        self.$XLSX.writeFile(new_workbook, self.currentComponent + '.xlsx')
                        self.$emit('update:exportWindowShow', false)
                    })
                    .catch(err => {
