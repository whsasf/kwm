...
<div>
    <div class="extendedWords-button-group">
        ...
        <div class="p2">
            <div class="p21">
                ...
                <i-page class="page" :total="itemZuCount" placement="bottom" :current="currentPage" :page-size="pageSize" :page-size-opts=[10,20,30,40,50,100] size="small" show-elevator show-total show-sizer @on-change="pageChange" @on-page-size-change="pageSizeChange" />
    ...
    <i-table class="basicWords-table" row-key="id" :load-data="handleLoadData" :columns="columns1" :data="extendedWordsZuItemData" :loading="tableLoading" @on-selection-change="handleSelectRow()" ref="table" stripe border @on-filter-change="handleFilter" @on-sort-change="handleSort">
        <template slot-scope="{ row }" slot="action">
            <div v-if="row.category">
                <i-button type="primary" size="small" style="margin-right: 5px" @click="editItem(row)">修改</i-button>
                <i-button :disabled="row.isTopicZu" type="warning" size="small" style="margin-right: 5px" @click="quitTopic(row['_id']['$oid'],row['topicWord'])">退出该主题</i-button>
                <i-button type="success" size="small" style="margin-right: 5px" @click="seeExpandTree(row)">查看拓词树</i-button>
            </div>
            <div v-else>
                <i-button type="error" size="small" style="margin-right: 5px" @click="dismiss(row['_id']['$oid'],row.word)">解散</i-button>
            </div>
        </template>
    </i-table>

    <i-page class="page" :total="itemZuCount" placement="top" :current="currentPage" :page-size="pageSize" :page-size-opts=[10,20,30,40,50,100] size="small" show-elevator show-total show-sizer @on-change="pageChange" @on-page-size-change="pageSizeChange" />
...
export default {
    name: 'extendedWordsTopic',
    data() {
        var global = this // 如果不这样， render 中找不到 正确的 this
        return {
            extendedWordsZuItemData: [],
            itemZuCount: 0,
            viewSelected: '主题视图',
            sortList: [{
                    'value': 'default',
    ...
    mounted() {
        ...
        self.fetchTopicWords(Params)
        // self.fetchProjectCategories();
        // self.fetchProjectUsageTags()
    },
    methods: {
        ...mapMutations(['changeSearchDisplay']),
        quitTopic: function (id, oldTopicWord) {
            //console.log(id, oldTopicWord)
            let self = this
            let Params = {
                'currentPage': self.currentPage,
                'pageSize': self.pageSize
        ...
        dismiss: function (uid,word) {
            ...
            self.axios({
                    method: 'delete',
                    url: self.baseurl + 'extendedWords/Topic/' + self.currentComponent,
        ...
        singleSearch: function (item) {
            ...
            let Params = {
                ...
                'pageSize': self.pageSize,
                // 'wordPart': item.word.toLowerCase(),
        ...
        handleUpdateExtendedWordItem: function (data2Update) {
            ...
            self.axios({
                ...
                .then(() => {
                    ...
                    let xid = ''
                    let xindex = ''
                    for (let ele in self.extendedWordsZuItemData) {
                        let lineData = self.extendedWordsZuItemData[ele]
                    ...
                    self.axios({
                        ...
                        .then(res => {
                            ...
                            //callback(xdata)
                            self.$set(self.extendedWordsZuItemData[xindex], 'children', xdata)
                            // console.log(self.extendedWordsItemData)
                        })
        ...
        fetchTopicWordsES: function (prams) {
            //console.log(prams)
            let self = this
            ...
            self.axios({
                ...
                .then(res => {
                    ...
                    self.tableLoading = false
                    if (res.data.count !== '') {
                        self.itemZuCount = res.data.count
                    }
                    //self.extendedWordsItemData = res.data.content
                    self.extendedWordsZuItemData = res.data.content
                    // console.log(self.extendedWordsItemData)
                })
                .catch(err => {
                    console.log(err)
        ...
        fetchTopicWords: function (prams) {
            //console.log(prams)
            let self = this
            self.tableLoading = true
            // 加上 聚合 的 参数
        ...
        deleteItems: function () {
            ...
            for (let element in self.selectedItemList) {
                let topicWord = ''
                if (self.selectedItemList[element]['topicWord']) {
                    topicWord = self.selectedItemList[element]['topicWord']
                }
                uids.push({
