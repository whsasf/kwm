...
<div class="url-url-select ivu-select ivu-select-visible ivu-select-single ivu-select-default" style="position: relative">
    <i-input v-model="searchItem" :clearable="true" search placeholder="输入url关键词进行模糊查询" @on-change="urlSearch" @on-search="batchSearch" @on-clear="resetInput" />
    <div ref='urlInputOptions' v-show="searchResult.length !== 0" class="url-url-select-option ivu-select-dropdown" x-placement="bottom-start" style="position: relative">
        <i-option @click.native="singleSearch([item['_id']['$oid'],item.highlight.rootUrl[0].replace(/<strong>/g,'').replace(/<\/strong>/g,'')])" style="text-align: left" v-for="(item,index) in searchResult" :tag="item['_id']['$oid']"  :value="index" :key="index" v-html="index + 1 + ': ' + item.highlight.rootUrl[0]  + '  -   ' + item['_id']['$oid'] "></i-option>
    </div>
</div>
</template>
        ...
        urlSearch: function () {
            ...
            if (query !== '') {
                ...
                let Params = {
                    ...
                    'showReturn': encodeURIComponent(['rootUrl']),
                    'highlight': encodeURIComponent(['rootUrl'])
                }
