    ...
    methods: {
        generateData: function (pageData2Export) {
            ...
            for (let ele in pageData2Export) {
                ...
                // 找出  urlIncludePath 和 urlExcludePath 哪个 更长？
                //console.log(lineData.urlIncludePath.length, lineData.urlExcludePath.length)
                let longer = lineData.urlIncludePath.length > lineData.urlExcludePath.length ? lineData.urlIncludePath.length : lineData.urlExcludePath.length
                //console.log('longer',longer)
                if (longer > 0) {
                    for (let ele2 = 0; ele2 < longer; ele2++) {
                        //let lineData2 = lineData.urlIncludePath[ele2]
                        let temp = []
                        temp.push(startId)
                        temp.push(lineData.rootUrl)
                        let selects = ['urlIncludePath', 'urlExcludePath']
                        for (let choose in selects) {
                            let lineData2 = lineData[selects[choose]][ele2]
                            if (lineData2) {
                                // console.log('lineData2',lineData2)
                                temp.push(lineData2.path)
                                temp.push(lineData2.type)
                            } else {
                                temp.push("")
                                temp.push("")
                            }
        ...
        modalOk() {
            ...
            let toPrint = []
            // 设置 header
            toPrint.push(['id', 'Url', '运行路径', '运行路径类型', '排除路径', '排除路径类型', '分类', '状态', '变更时间'])
            toPrint.push(['', '', '', '', '', '', '', '', ''])
            ...
            if (self.selected3 === '当前选中' || self.selected3 === '当前页') {
                ...
                self.$XLSX.utils.book_append_sheet(new_workbook, worksheet, "Urls数据")
