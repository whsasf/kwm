...
export default {
    ...
    methods: {
        ...
        modalOk() {
            ...
            let toPrint = []
            // 设置 header
            toPrint.push(['id', '无效词', '时间', '操作人', '前状态', '来源'])
            toPrint.push(['', '', '', '', '', ''])
            ...
            if (self.selected3 === '当前选中' || self.selected3 === '当前页') {
                ...
                self.$XLSX.utils.book_append_sheet(new_workbook, worksheet, "无效词数据")
            ...
            if (self.selected3 !== '当前选中' && self.selected3 !== '当前页') {
                ...
                self.axios({
                    ...
                    .then(res => {
                        ...
                        let worksheet = self.$XLSX.utils.aoa_to_sheet(toPrint)
                        let new_workbook = self.$XLSX.utils.book_new()
                        self.$XLSX.utils.book_append_sheet(new_workbook, worksheet, "无效词典数据")
                        self.$XLSX.writeFile(new_workbook, self.currentComponent + '.xlsx')
                        self.$emit('update:exportWindowShow', false)
                    })
                    .catch(err => {
